...
let defaultCode = `/// Thanks for using this test javascript text editor. 
// To read more about me, go to my about page :)

// Starts the function!
function TEST() {
    // Lets x be a random number 
    let x = Math.floor(Math.random(100) * 12);
    // Lets helper be a random number
    let helper= Math.floor((Math.random(100) * 12));
    
    // if else
    
    if( x > helper) {
         console.log("X was larger:",  x , "| helper: ", helper);
    } else if( x < helper)  {
        console.log("Helper was larger: ", helper, "| X: ", x);
    } else {
        console.log("X and the Helper are equal!");
    }
    return;
}
// Runs the function
